...
public class Drug extends BaseEntity {
    ...
    @Column(length=4000, name="item_eng_name")
    private String itemEngName; //제품 영문명

    @Column(length=4000, name="entp_name")
    private String entpName; //업체명

    @Column(length=4000, name="entp_seq")
    private String entpSeq; //업체 식별자

    @Column(length=65535, name="item_permit_date")
    private String itemPermitDate; //특허 허가일 yyyymmdd

    @Column(length=65535, name="spclty_pblc")
    private String spcltyPblc; //약품 구분(전문 or 일반)

    @Column(length=65535, name="prduct_type")
    private String prductType; //약품 타입

    @Column(length=65535, name="item_ingr_name")
